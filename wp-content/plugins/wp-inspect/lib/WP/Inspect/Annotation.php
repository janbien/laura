...
/**
 * @package WP_Inspect
 * @subpackage WP_Inspect_Annotation
 */
class WP_Inspect_Annotation {
    ...
    function get_return()
    {
        if (null !== $this->return) {
            ...
            if (false === strpos($this->return->type, '|') &&
                ...
                if (!preg_match('/exit|die|bool(ean)|string|null|float|int(eger)?|float|dec(imal)?|object|resource|array|callback/i', $this->return->type)) {
