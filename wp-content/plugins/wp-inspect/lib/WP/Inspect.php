...
/**
 * The WordPress Inpsect Plugin.
 * 
 * @package WP_Inspect
 */    
class WP_Inspect extends WP_Inspect_Plugin {
    ...
    protected function __construct()
    {
        ...
        define('WPI_CONTEXT_HIDDEN', self::CONTEXT_HIDDEN);
        define('WPI_CONTEXT_HEADER', self::CONTEXT_HEADER);
        define('WPI_CONTEXT_MIDDLE', self::CONTEXT_MIDDLE);
        define('WPI_CONTEXT_FOOTER', self::CONTEXT_FOOTER);
    }
    ...
    {
        if (is_admin()) {
            wp_enqueue_script('bs-loophole', plugins_url('assets/js/bs-loophole.min.js', WPI_PLUGIN_FILE), array('jquery'), null, true);
        }
        
        if ($this->is_enabled()) {
            wp_enqueue_style('qtip', plugins_url('assets/css/jquery.qtip.min.css', WPI_PLUGIN_FILE));        
            wp_enqueue_script($this->meta->slug, plugins_url("assets/js/{$this->meta->slug}.min.js", WPI_PLUGIN_FILE), array('jquery'), null, true);                    
            
        } else {
            wp_enqueue_script($this->meta->slug, plugins_url("assets/js/{$this->meta->slug}.min.js", WPI_PLUGIN_FILE), array('jquery'), null, true);                    
        }
        
        wp_localize_script($this->meta->slug, $this->meta->abbr, array(
            'ajax_url' => admin_url("admin-ajax.php?{$this->meta->slug}=true")
           ,'plugin_url' => plugins_url('', WPI_PLUGIN_FILE) . DS
           ,'request_key' => $this->get_request_key()
    ...
    {
        if (is_admin()) {
            wp_enqueue_style($this->meta->slug . '-screen-admin', plugins_url('assets/css/screen-admin.min.css', WPI_PLUGIN_FILE));          
        } else {
            wp_enqueue_style($this->meta->slug . '-screen', plugins_url('assets/css/screen.min.css', WPI_PLUGIN_FILE));            
        }        
    }
    ...
    {
        if (!$this->is_user_legit())
            return;
        
        $uri = plugins_url("assets/images/{$this->meta->abbr}-grey.svg", WPI_PLUGIN_FILE);
        
        $wp_admin_bar->add_node(array(
            'id' => $this->meta->slug,
            'title' => sprintf('<img src="%s" />', $uri),
            'href' => admin_url('tools.php?page=' . $this->meta->slug),
            'meta' => array(
        ...
        if ($this->is_enabled() && !$this->is_tools_page()) {        
            $wp_admin_bar->add_node(array(
                'parent' => $this->meta->slug,
                'id' => sprintf('%s-search', $this->meta->slug),
        ...
        if (!$this->is_tools_page()) {
            $wp_admin_bar->add_node(array(
                'parent' => $this->meta->slug,
                'id' => sprintf('%s-enabled', $this->meta->slug),
    ...
    {
        register_setting($this->meta->slug, $this->meta->slug, array($this, 'settings_validate'));
    }
    ...
    function register_actions($actions)
    {
        return array_merge(
            array(
              'settings' => '<a href="' . get_bloginfo('wpurl') . sprintf('/wp-admin/tools.php?page=%s">Settings</a>', $this->meta->slug)
            ),
            $actions
	);
    }
    ...
    function settings_page()
    {
        ...
        foreach ($roles as $key => $role) {
            ...
            $role->enabled = false;
            $role->key = $key;
            if (false !== ($index = array_search($key, $data['roles']))) {
                $role->enabled = true;
                unset($data['roles'][$index]);
            }
            $data['roles'][] = $role;
        }
    ...
    {
        return settings_fields($this->meta->slug);
    }
     ...
     * @return array
     */
    function settings_validate($args = array())
    {        
        print_e($args);
    ...
    {
        return get_option($this->meta->slug);        
    }
    ...
    protected function get_settings()
    {
        ...
        if (null === $settings) {
            ...
            // settings forcefully removed?
            if (empty($settings)) {
                $settings += (array)$this->get_settings_default();
            } else {
    ...
    protected function get_settings_default()
    {
        ...
        if (null === $settings) {
            ...
            $settings->roles = array('administrator');

            $settings->mode = new stdClass();
            $settings->mode->trace = true;        
            $settings->mode->actions = true;
            $settings->mode->filters = true;
            $settings->mode->off_canvas = false;
            $settings->mode->ignore = implode("\n", array(
                'gettext',
            ...
            $settings->mode->watch = '';
            $settings->mode->ctx_hidden = 0;
            $settings->mode->ctx_header = self::CONTEXT_HEADER;
            $settings->mode->ctx_middle = self::CONTEXT_MIDDLE;
            $settings->mode->ctx_footer = self::CONTEXT_FOOTER;

            foreach (array('public', 'admin') as $mode) {
                foreach ($settings->mode as $key => $val) {
    ...
    protected function get_settings_empty()
    {
        ...
        if (null === $settings) {
            ...
            $settings = $this->get_settings_default();
    ...
    function track_hook()
    {
        ...
        if (false !== strpos($tag, 'rest_url') && is_rest()) {     
            return $this->unload();
        }
    ...
    protected function create_hook($tag, $args = array())
    {
        ...
        static $hook = null, $index = null, $time = null, $prev = null;
        ...
        if (null !== $prev)
            $prev->elapsed = sprintf('%.10f', $now - $time);
        ...
        $clone->value = wpi_parse_arg(array_shift($args));
    ...
    {
        return // Something happend on the way to heaven.
               $this->abort ||

               // Ignore logging-in attempts due to redirects/headers.
               is_user_logging_out() ||
    ...
    {
        return null !== $this->settings && 1 == $this->settings->enabled && $this->is_user_legit();
    }
    ...
    {
        return is_admin() && (false !== stripos($_SERVER['REQUEST_URI'], 'tools') && $this->meta->slug === @$_GET['page']);
    }
     ...
     * @return boolean
     */
    protected function is_user_legit()
    {
        static $legit = null;
        ...
        if (null === $legit) {
            ...
            if (empty($roles))
                ...
                $legit = !!count(array_intersect(get_user_roles(), $roles));            
        }
    ...
    {     
        switch ($node->nodeType) {
            ...
            case 3:
                ...
                $this->node_add_class($element, $this->meta->abbr);                
                ...
                foreach ($keys as $key) {
                    ...
                    // This is capturing the last hook's id, when it probably 
                    // should be the first... (cs)
                    if ('action' === $hook->type) {
                        ...
                        $this->set_node_attrs($hook->node, array(
                            'id' => $this->get_field_id($hook->index, true)
                           ,'class' => $this->get_field_id($hook->type, true)
                           ,'title' => $hook->tag
                           ,"data-{$this->meta->abbr}" => $this->encode_keys($hook->index)                          
                        ));
                        
                    } else {

                        $this->node_add_class($element, $this->get_field_id($hook->index, true));
                        $class = $this->get_field_id('filter', true);
                        ...
                        if (!$this->node_has_class($element, $class)) {                                      
                            ...
                            $marker->setAttribute('class', $this->get_field_id('marker', true));
                        }
                    }
                ...
                if ($element->hasAttribute("data-{$this->meta->abbr}")) {
                    $ids = array_merge(
                        $this->decode_keys($element->getAttribute("data-{$this->meta->abbr}"))
                       ,$ids
                    );
                }

                sort($ids);
                $element->setAttribute("data-{$this->meta->abbr}", $this->encode_keys($ids));
    ...
    protected function filter_node($node)
    {
        ...
        $this->node_add_class($node, $this->meta->slug);
        ...
        foreach ($keys as $key) {
            ...
            if ('filter' === $hook->type) {
                ...
                $this->node_add_class($node, $this->get_field_id($hook->index, true));                
                $class = $this->get_field_id('filter', true);
        ...
        sort($ids);
        $node->setAttribute("data-{$this->meta->abbr}", $this->encode_keys($ids));                
    }
    ...
    protected function ignore_node($tag)
    {
        ...
        if (null === $tags) {
            $tags = apply_filters('wpi_ignore_nodes', array('script', 'style', 'textarea', 'input'));
        ...
        if (preg_match_all('/<(?![!\/\\\])([^ >]+)[^>].*?>/ms', $buffer, $matches)) {
            ...
            for ($i = 0, $c = count($nodes); $i < $c; ++$i) {
                ...
                if (!empty($keys)) {
                    ...
                    $omega = str_replace('<' . $tag, '<' . $tag . ' data-' . $this->meta->abbr . '="' . $this->encode_keys($keys) . '"', $omega);
     ...
     * WPI doens't know what context it render's it's keys, pre-filtering is
    ...
    protected function post_filter_body($dom)
    {
        ...
        $class = preg_split('/\s+/', preg_replace("/{$this->meta->abbr}(-.+)?/", '', $element->getAttribute('class')));
        $element->removeAttribute("data-{$this->meta->abbr}");
        ...
        if ($this->settings->off_canvas) {
            ...
            $this->set_node_attrs($node, array(
                'id' => $this->get_field_id('offcanvas', true)
               ,'class' => $this->meta->abbr
            ));
            $child = $this->get_child_node($node);        
        }
     ...
     * performance when it comes to large inline-scripts. To prevent WPI from 
    ...
    {
        if (!$this->is_enabled() || is_ajax() || is_rest())
            return $buffer;
    ...
    protected function get_cache_file($key = null)
    {
        ...
        $file = WPI_PLUGIN_DIR . 'cache' . DS . $key . '.php';                
        return $file;
    }
    ...
    protected function clear_cache($force = false)
    {
        ...
        $files = glob(WPI_PLUGIN_DIR . 'cache' . DS . '*');
    ...
    {
        $tpl = WPI_PLUGIN_DIR . DS . 'tpl' . DS . $tpl;
        parent::render($tpl, $data);
    }
    /**#@-*/
        ...
        if (null === ($detail = $this->get_cache($cache_key))) {
            ...
            if (@isset($hook->caller)) {
                ...
                if (version_compare(PHP_VERSION, '5.3.0') >= 0) {
                    $block = new WP_Inspect_DocComment(implode("\n", $buffer));
                    $hook->annotation = new WP_Inspect_Annotation($block);
                }                
            }
    ...
    protected function ajax_enable()
    {
        ...
        // TODO(cs) This is ef'd up... fix it.
        $settings->enabled = 'true' === $enabled ? 1 : 0;
       
        update_option($this->meta->slug, (array)$settings);
        echo 1;
    }
    ...
    {        
        ...
        if ($error instanceof Exception) {
            ...
            if (is_wp_error($error)) {
                $error = new WP_Inspect_Exception(
                    $error->get_error_message(),
                    $error->get_error_code()                
                );
            } else {
                $error = new WP_Inspect_Exception($message, $error, $file, $line);
            }
    ...
    {
        if (false === $this->abort || !is_ajax(true)) {
    ...
    function activate()
    {        
        add_option($this->meta->slug, (array)$this->get_settings_default());
    }
